...
const getArticles = (req, res) => {
	...
	//this part only return all articles
		...
		.skip(pageOptions.page*pageOptions.limit).limit(pageOptions.limit).exec(function(e, docs){
            if (e){
                console.log(err)
                res.sendStatus(500)
                return
...
const addArticle = (req,res) =>{
	const username = req.username
	const newArticle = new Article({author:username,text:req.body['text'],date:new Date(),comments:[],img:req.fileurl})
...
module.exports = (app) => {
	...
	app.post('/article',isLoggedIn,uploadImage("title"),addArticle)
